...
def test_insert_get():
    ...
    time_list.append(datetime.now())
    first_result = db.tables[0].select(db.tables[0].fields, [], start_time=time_list[1], end_time=time_list[2])
    assert len(first_result) == 2
    assert first_result[0].fields_values_dict["zhenya1"] == 50200
    second_result = db.tables[0].select(db.tables[0].fields, [], start_time=time_list[0], end_time=time_list[2])
    assert len(second_result) == 3


def test_delete_get():
    db.tables[0].delete()
    result = db.tables[0].select(db.tables[0].fields, [], start_time=time_list[0], end_time=time_list[2])
    assert len(result) == 3
    time_list.append(datetime.now())
    time_list.append(datetime.now())
    empty_result = db.tables[0].select(db.tables[0].fields, [], start_time=time_list[3], end_time=time_list[4])
