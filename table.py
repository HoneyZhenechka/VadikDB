...
class Table:
    ...
    def load_from_file(self, table_name, path):
        ...
        for key, value in table_meta["fields"].items():
            ...
            self.table_matrix[0].append(key)
        for i in data_json[table_name][self.table_matrix[0][0]]:
        ...
        for i in range(len(temp_list)):
            ...
            for j in range(len(temp_list[i])):
                self.table_matrix[records_index].append(temp_list[i][j])
                records_index = records_index + 1

    def get_type(self, field):
        return self.types[self.table_matrix[0].index(field)]
    ...
    def save_to_file(self, path):
        ...
        with open(path / "data.json", "r") as data_file:
            data_json = json.load(data_file)
        fields_dict = {}
        for i in range(len(self.table_matrix[0])):
            fields_dict[self.table_matrix[0][i]] = self.types[i]
        data_dict = {}
        temp_list = []
        for i in range(len(self.table_matrix[0])):
            temp_list.append([])
            for j in range(1, len(self.table_matrix), 1):
                temp_list[i].append(self.table_matrix[j][i])
        for i in range(len(self.table_matrix[0])):
            data_dict[self.table_matrix[0][i]] = temp_list[i]
